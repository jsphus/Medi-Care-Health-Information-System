    ...
    const content = `
        ...
        <div style="margin-bottom: 2rem; padding: 1.5rem; background: #f9fafb; border-radius: 8px; border-left: 4px solid var(--primary-blue);">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                <div>
                    <p style="margin: 0.5rem 0; font-size: 0.875rem;"><strong style="color: var(--text-primary);">Appointment ID:</strong> <span style="color: var(--text-secondary);">${record.appointment_id || record.appt_id || 'N/A'}</span></p>
        ...
        <div style="margin-bottom: 2rem; padding: 1.5rem; background: #f0fdf4; border-radius: 8px; border-left: 4px solid #10b981;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 1rem;">
                ...
                <div>
                    <p style="margin: 0.5rem 0; font-size: 0.875rem;"><strong style="color: var(--text-primary);">Created:</strong> <span style="color: var(--text-secondary);">${formatDate(record.med_rec_created_at || record.created_at)}</span></p>
                    <p style="margin: 0.5rem 0; font-size: 0.875rem;"><strong style="color: var(--text-primary);">Updated:</strong> <span style="color: var(--text-secondary);">${formatDate(record.med_rec_updated_at || record.updated_at)}</span></p>
                </div>
            </div>
            
            <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--border-color);">
                <h4 style="margin-bottom: 0.75rem; color: var(--text-primary); font-size: 1rem; font-weight: 600;">Diagnosis</h4>
                <p style="white-space: pre-wrap; margin: 0; color: var(--text-primary); padding: 0.75rem; background: white; border-radius: 4px; border: 1px solid var(--border-color);">${record.med_rec_diagnosis || record.diagnosis || 'N/A'}</p>
            </div>
            
            <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--border-color);">
                <h4 style="margin-bottom: 0.75rem; color: var(--text-primary); font-size: 1rem; font-weight: 600;">Prescription</h4>
                <p style="white-space: pre-wrap; margin: 0; color: var(--text-primary); padding: 0.75rem; background: white; border-radius: 4px; border: 1px solid var(--border-color);">${record.med_rec_prescription || record.prescription || 'None'}</p>
            </div>
        </div>
    `;
